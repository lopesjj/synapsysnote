    ...
    try {
      ...
      for (const [index, node] of ordered.entries()) {
        ...
        try {
          const appId =
            node.type === "database"
              ? await importDatabase({ notion, workspaceId, jobId, job, node, parents, idMap, progress })
              : await importPage({ notion, workspaceId, jobId, job, node, parents, idMap, progress });
...
async function importPage(args: ImportArgs): Promise<string> {
  const { notion, workspaceId, jobId, job, node, parents, idMap, progress } = args;
  ...
  const parentNotionId = job.options.preserveHierarchy ? (parents.get(node.id) ?? null) : null;
  ...
  await ref.set(
    {
      title: node.title,
      icon: node.icon ?? "📄",
      notebookId: job.targetNotebookId,
