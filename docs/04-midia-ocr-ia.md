...
Duas portas de entrada, mesma implementação: o callable `transcribeAudio`, que o
app chama logo após o upload para manter a latência baixa enquanto o usuário
espera, e o gatilho `transcribeOnUpload`, rede de segurança para áudios que
cheguem por outro caminho. O gatilho detecta trabalho já feito e sai.
...
de lixeira, e `restorePage` limpa `deletedAt`.

## 5. Inventário de funções

| Função | Tipo | Gatilho / uso |
| --- | --- | --- |
| `listNotionTree` | callable | árvore do wizard |
| `startNotionImport` | callable | enfileira o job |
| `processNotionImportJob` | Firestore | worker (60 min, 1 GiB) |
| `disconnectNotion` | callable | revoga e apaga o token |
