...
/workspaces/{workspaceId}
  ...
  /notebooks/{notebookId}
      name, emoji, color, description, order, createdAt, updatedAt
...
**`path[]` materializado.** Guardar a cadeia de ancestrais permite carregar uma
subárvore inteira com `where('path', 'array-contains', pageId)`, sem recursão de
leituras. O custo é reescrever `path` ao mover uma página — operação rara.
