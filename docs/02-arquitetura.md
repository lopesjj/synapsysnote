...
| Importação do Notion | callable → worker em background | worker simulado no browser |
...
adaptador, assina todas as coleções e expõe estado derivado: árvore de páginas,
lixeira, tags globais, job de importação ativo. Componentes chamam
`useWorkspace()` e nunca sabem qual backend responde.
...
Vercel), caminho de arquivo, ou Application Default Credentials. Usado apenas
pelo callback OAuth, que precisa escrever num caminho fechado às regras.
...
Agrupa digitação em uma escrita (700 ms de ociosidade) com teto de 4 s para quem
digita sem parar, e faz *flush* em `visibilitychange`, `beforeunload` e
desmontagem. Expõe uma máquina de estados (`idle → dirty → saving → saved`) que
o indicador de status consome diretamente.
...
Google OAuth e e-mail/senha via Firebase Auth, com um modo convidado que ativa a
demonstração local. Os módulos de `firebase/auth` são importados dinamicamente
para não entrarem no bundle inicial.
...
flash de canvas claro no modo escuro.
