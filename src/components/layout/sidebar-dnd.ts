...
export type DropPlan =
  /** New notebook order, in position order. */
...
export function planSidebarDrop(
  ...
  if (active.kind === "notebook") {
    ...
    const to = notebooks.findIndex((notebook) => notebook.id === targetNotebookId);
    if (from < 0 || to < 0 || from === to) return null;

    return {
      kind: "reorder-notebooks",
      notebookIds: moveItem(notebooks, from, to).map((notebook) => notebook.id),
    };
  }
