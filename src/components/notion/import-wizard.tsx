...
export function ImportWizard({
  ...
  const resolvedNotebookId = targetNotebookId ?? notebooks[0]?.id ?? null;
      ...
      <DialogHeader
        ...
        description={
          integration?.connected
            ? `Conectado a ${integration.workspaceName}. Selecione o que deseja trazer.`
            : "Conecte um workspace do Notion para listar páginas, cadernos e bases de dados."
        }
      />
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-5 px-8 py-14 text-center">
      <div className="space-y-1.5">
        <h3 className="text-[15px] font-semibold text-ink">Conectar workspace do Notion</h3>
        <p className="mx-auto max-w-md text-[12.5px] leading-relaxed text-muted">
          Usamos OAuth 2.0. O token é criptografado e guardado apenas no servidor, em
          <code className="mx-1 rounded bg-[var(--surface-2)] px-1 py-0.5 font-mono text-[11px]">
            /workspaces/&#123;id&#125;/integrations/notion
          </code>
          — o navegador nunca vê a credencial.
        </p>
      </div>
      ...
      <p className="text-[11px] text-faint">
        Você escolhe quais páginas compartilhar durante a autorização.
      </p>
    </div>
  );
}
...
function PreviewStep({
  ...
  const toggles = [
    ...
    },
    {
      ...
      title: "Preservar hierarquia de páginas",
      description: "Páginas-filhas do Notion viram páginas com parentPageId no Firestore.",
    },
    {
      key: "runOcr" as const,
  ...
  return (
    <div className="space-y-5 px-5 py-5">
      ...
      <div className="space-y-2">
        <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-faint">Destino</p>
