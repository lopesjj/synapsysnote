...
function BlockNode({ block }: { block: AppBlock }) {
  switch (block.type) {
    case "heading_1":
      return (
        <h2 className="pt-2 text-[1.5em] font-semibold tracking-[-0.02em]">
          <Spans spans={block.richText} />
        </h2>
      );
    case "heading_2":
      return (
        <h3 className="pt-1.5 text-[1.25em] font-semibold tracking-[-0.015em]">
          <Spans spans={block.richText} />
        </h3>
      );
    case "heading_3":
      return (
        <h4 className="pt-1 text-[1.1em] font-semibold">
          <Spans spans={block.richText} />
        </h4>
      );
    ...
    case "image":
      return block.media?.url ? (
        ...
        <img
          ...
          className="max-h-64 w-auto rounded-[var(--radius-md)] border border-[var(--border)] object-contain"
          loading="lazy"
        />
      ) : (
